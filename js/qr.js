(function () {
  ...
  function qrImageDataUri(value) {
    ...
    qr.make();
    if (typeof qr.createDataURL === "function") return qr.createDataURL(5, 4);
  ...
  async function decodeQrImage(file) {
    ...
    if ("BarcodeDetector" in window) {
      try {
        ...
        if (nativeCodes && nativeCodes.length && nativeCodes[0].rawValue) {
          return nativeCodes[0].rawValue.trim();
        }
      } catch (error) {
    ...
    if (typeof window.jsQR !== "function") throw new Error("The local QR decoder is unavailable.");
